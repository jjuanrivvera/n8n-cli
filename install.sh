#!/bin/sh
# n8nctl installer for macOS and Linux.
#
...
#   N8NCTL_VERSION=v1.2.3   pin a version (default: the latest release)
...
REPO="jjuanrivvera/n8n-cli"
BINARY="n8nctl"
VERSION="${N8NCTL_VERSION:-}"
...
if [ -z "$VERSION" ]; then
  ...
  [ -n "$VERSION" ] || die "could not determine the latest release; set N8NCTL_VERSION"
fi
