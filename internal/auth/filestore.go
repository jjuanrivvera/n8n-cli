...
// fileStore is the encrypted-file fallback used when no OS keyring is available. Keys are
// AES-256-GCM encrypted in a 0600 file. The encryption key comes from $N8NCTL_KEYRING_PASSWORD
...
// protection, and this fallback only exists so headless hosts still avoid plaintext keys.
...
func deriveKey() ([]byte, error) {
	salt := []byte("n8nctl-credentials-v1")
	if pw := os.Getenv("N8NCTL_KEYRING_PASSWORD"); pw != "" {
		return scrypt.Key([]byte(pw), salt, 1<<15, 8, 1, 32)
	}
	host, _ := os.Hostname()
	seed := fmt.Sprintf("n8nctl-fallback|%s|%d", host, os.Getuid())
...
func decrypt(b64 string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("decrypt failed (wrong $N8NCTL_KEYRING_PASSWORD or host changed): %w", err)
	}
