...
func TestFileStore_RoundTrip(t *testing.T) {
	...
	assert.NotContains(t, string(raw), "token-a", "key must be encrypted at rest")
...
func TestFileStore_PasswordKey(t *testing.T) {
	t.Setenv("N8NCTL_KEYRING_PASSWORD", "correct horse battery staple")
	dir := t.TempDir()
	fs := newFileStore(dir)
	...
	t.Setenv("N8NCTL_KEYRING_PASSWORD", "wrong password entirely")
	_, err = fs.Get("x")
	assert.Error(t, err)
}
