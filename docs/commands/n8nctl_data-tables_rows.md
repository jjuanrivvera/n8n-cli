...
```
n8nctl data-tables rows <tableId> [flags]
```

### Options

```
      --filter string   filter as a JSON string
  -h, --help            help for rows
      --limit int       max rows to return
      --search string   full-text search across string columns
```
